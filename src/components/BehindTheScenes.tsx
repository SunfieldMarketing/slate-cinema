...
const fallbackStills = [
  { url: '/images/portfolio-production.webp', label: 'On Set', desc: "Whether it's in the universe or metaverse our team shows up.", span: 'tall' as const },
  { url: '/images/portfolio-brand.webp', label: 'The Edit Bay', desc: 'Frame-by-frame assembly with an editor who thinks in story beats.' },
  { url: '/images/portfolio-social.webp', label: 'Color Suite', desc: 'A signature grade that makes your brand recognizable in any feed.' },
  { url: '/images/portfolio-event.webp', label: 'Sound Stage', desc: 'Mix, score, and sound design tuned for sound-on and sound-off.', span: 'wide' as const },
]
...
export default function BehindTheScenes({ data }: { data?: HowItWorksPage['behindTheScenes'] }) {
  ...
  return (
    <section ref={ref} data-cms-global="how-it-works-page" className="relative w-full bg-ink overflow-hidden py-24 md:py-28">
      <div className="relative z-10 w-full max-w-7xl mx-auto px-5 sm:px-8">
        <div className="bts-head text-center mb-12 md:mb-14 max-w-2xl mx-auto">
          <span className="inline-flex items-center gap-3 font-mono text-[10px] sm:text-[11px] tracking-[0.3em] text-[#00AEEF] uppercase mb-4">
            <span className="w-8 h-px bg-[#00AEEF]/40" /> <span data-cms-field="behindTheScenes.eyebrow">{eyebrow}</span>
          </span>
          <h2 data-cms-field="behindTheScenes.headline" className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight text-white leading-[1.05] mb-4">
            {headline}
          </h2>
          <p data-cms-field="behindTheScenes.subhead" className="text-white/55 text-sm sm:text-base font-light">
            {subhead}
          </p>
        </div>
          ...
          {stills.map((s, i) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 pointer-events-none">
                <h3 data-cms-field={`behindTheScenes.stills.${i}.label`} className="font-mono text-white font-bold text-sm tracking-wide uppercase mb-1.5">{s.label}</h3>
                <p data-cms-field={`behindTheScenes.stills.${i}.desc`} className="font-serif-accent italic text-white/60 text-sm leading-relaxed max-h-0 opacity-0 group-hover:max-h-16 group-hover:opacity-100 transition-all duration-500 overflow-hidden">
                  {s.desc}
                </p>
              </div>
            </div>
