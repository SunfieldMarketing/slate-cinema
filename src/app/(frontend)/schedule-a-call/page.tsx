...
function CallFraming({ copy }: { copy: ReadyToTalkGlobal | null }) {
  ...
  return (
    <section data-cms-global="ready-to-talk" className="relative w-full overflow-hidden pt-16 pb-4 md:pt-20">
      <div className="relative z-10 w-full max-w-3xl mx-auto px-5 sm:px-8 text-center">
        ...
        <p data-cms-field="description" className="text-white/60 font-light text-base sm:text-lg leading-relaxed">
          {description}
        </p>
          ...
          {prepItems.map((p, i) => {
            ...
            return (
              <div key={p.label} className="rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-md p-6">
                ...
                <div data-cms-field={`prepItems.${i}.label`} className="text-white font-bold text-sm mb-1.5">{p.label}</div>
